...
-/
axiom shiftInvariantSigma : MeasurableSpace (Ω[α])

axiom shiftInvariantSigma_le : shiftInvariantSigma ≤ (inferInstance : MeasurableSpace (Ω[α]))

axiom mem_shiftInvariantSigma_iff (s : Set (Ω[α])) :
    @MeasurableSet _ shiftInvariantSigma s ↔ isShiftInvariant s
...
lemma fixedSubspace_closed {μ : Measure (Ω[α])} [IsProbabilityMeasure μ]
  ...
  sorry
  -- The fixed-point set of a continuous linear map is closed
...
-/
axiom condexpL2 {μ : Measure (Ω[α])} [IsProbabilityMeasure μ]
    (m : MeasurableSpace (Ω[α])) : Lp ℝ 2 μ →L[ℝ] Lp ℝ 2 μ
