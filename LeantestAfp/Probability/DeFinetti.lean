...
/-- An infinite family of random variables `X : ℕ → Ω → α` is exchangeable if the
finite-dimensional distributions are invariant under permutations.  The precise
mathematical content will follow the standard definition from probability
theory.

`TODO`: replace the `sorry` with the actual definition once the blueprint is in place.
-/
def Exchangeable (μ : Measure Ω) (X : ℕ → Ω → α) : Prop :=
  sorry

/-- The tail `σ`-algebra associated with the stochastic process `X`.  In the
final development this will be defined as the intersection over `n` of the
`σ`-algebras generated by the coordinates from `n` onwards.

`TODO`: formalise this using mathlib's `MeasureTheory` infra for `MeasurableSpace`s.
-/
...
theorem deFinetti
    ...
    ∃ (ξ : DirectingMeasure Ω α μ),
      -- TODO: express that `X` is conditionally i.i.d. with law `ξ.kernel`.
      True :=
  sorry
