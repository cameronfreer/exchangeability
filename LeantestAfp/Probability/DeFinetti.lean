...
theorem exchangeable_iff_fullyExchangeable {μ : Measure Ω} {X : ℕ → Ω → α}
  ...
  · intro hexch
    ...
    have h_unique : μ_X = μ_Xπ := by
      sorry
      -- This requires:
      -- i) Proving projective consistency of finite-dimensional distributions
      --    (follows from definition of product σ-algebra)
      -- ii) Invoking uniqueness from ProbabilityTheory.Kernel.eq_traj or similar
      -- iii) The uniqueness statement says: if two measures on ℕ → α have the same
      --     finite-dimensional marginals, they are equal
