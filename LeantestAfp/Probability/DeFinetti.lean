...
theorem exchangeable_iff_fullyExchangeable {μ : Measure Ω} {X : ℕ → Ω → α}
  ...
  · intro hexch
    intro π
    -- Use Ionescu-Tulcea to extend finite-dimensional distributions
    sorry
    -- Proof outline:
    -- 1. The finite-dimensional distributions satisfy projective consistency
    --    (this follows from exchangeability)
    -- 2. By Ionescu-Tulcea/Kolmogorov extension, there exists a unique measure
    --    on ℕ → α with these finite-dimensional marginals
    -- 3. By uniqueness, permuting by π gives the same measure
    -- 4. Therefore the process is fully exchangeable
