...
For the shift on path space, the shift-invariant $\sigma$-algebra coincides with the tail $\sigma$-algebra:
\[
\mathcal{I} = \cF_\infty = \bigcap_n \sigma(X_n, X_{n+1}, \ldots)
\]
...
\item Conditionally on the tail (= invariant $\sigma$-algebra), coordinates are i.i.d.
\end{itemize}
