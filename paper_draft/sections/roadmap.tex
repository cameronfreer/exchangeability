...
Interprets the sequence as a dynamical system with the shift operator. The Mean Ergodic Theorem gives $L^2$ convergence of Ces\`{a}ro averages to the projection onto shift-invariant functions.

\paragraph{Conceptual unity.}
Despite their different machinery, all three proofs share a common structure: each extracts the directing measure $\nu$ via a \emph{limit theorem} applied to some ``asymptotic summary'' of the sequence.
\begin{itemize}
\item Martingale: $\nu$ is the limit of conditional laws under reverse martingale convergence.
\item $L^2$: $\nu$ is built from limits of block averages (an analytic law-of-large-numbers flavor).
\item Koopman: $\nu$ is built from ergodic projections (Ces\`{a}ro averages converge to invariant functions).
\end{itemize}
In each case, symmetry (contractability) provides the regularity that makes the limit exist, and the limit encodes the ``latent parameter'' that governs the mixture.
