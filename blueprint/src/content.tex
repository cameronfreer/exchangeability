...
the Stieltjes construction supplies the candidate kernel, and a pair of bridge
lemmas identifies its push-forward and integral with the conditional expectation
on the tail $\sigma$-algebra. The shared common-ending theorem then lifts the
indicator-level identity to the full product $\sigma$-algebra.
...
\begin{lemma}[Directing measure is probability (L$^2$ version)]
  ...
  Monotonicity, bounds, and right-continuity of the underlying CDF are inherited
  directly from mathlib's \texttt{stieltjesOfMeasurableRat} construction.
\end{lemma}
