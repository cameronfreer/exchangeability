...
\begin{definition}[Block average]
  ...
  \leanok
  The \emph{block average} $A_{n,m,k}(f)(\omega) = \frac{1}{n} \sum_{j=0}^{n-1} f(\omega_{mn+k+j})$
  averages $f$ over the $k$-th block of size $n$ starting at position $mn$.
\end{definition}

\begin{lemma}[Block averages converge to conditional expectation]
  \label{lem:blockAvg_tendsto}
  \lean{Exchangeability.DeFinetti.ViaKoopman.blockAvg_tendsto_condExp}
  \leanok
  \uses{def:blockAvg}
  For a shift-invariant measure, block averages converge a.e.\ to the conditional
