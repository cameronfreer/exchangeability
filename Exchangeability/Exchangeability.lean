...
lemma measurable_prefixProj (n : ℕ) :
  ...
  change Measurable fun x : ℕ → α => x i
  simpa using (measurable_pi_apply i :
...
/-- Wrapper set of all prefix cylinders with measurable base. -/
...
theorem measure_eq_of_fin_marginals_eq {μ ν : Measure (ℕ → α)}
  ...
  have h_univ : μ Set.univ = ν Set.univ := by
    simpa [Measure.map_apply_of_aemeasurable
      ...
      using h 1 Set.univ measurableSet_univ
  apply Measure.ext_of_generate_finite
      (μ:=μ) (ν:=ν) (C:=prefixCylinders (α:=α))
...
lemma pathLaw_map_prefix_perm (μ : Measure Ω) (X : ℕ → Ω → α)
  ...
  have hpath := pathLaw_map_prefix (α:=α) μ X hX n
  have hcomp' :
...
theorem exchangeable_iff_fullyExchangeable {μ : Measure Ω}
  ...
  · intro hEx π
    ...
    have hEq :=
      measure_eq_of_fin_marginals_eq (α:=α)
