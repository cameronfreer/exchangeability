...
theorem exchangeable_iff_fullyExchangeable {μ : Measure Ω} {X : ℕ → Ω → α}
  ...
  · intro hexch π
    ...
    have h_unique : μ_X = μ_Xπ := by
      -- Strategy: Use Measure.ext and show equality on all measurable sets
      -- by leveraging h_marginals on cylinder sets
      ext S hS
      
      -- To prove: μ_X S = μ_Xπ S for all measurable S
      -- The challenge: S is an arbitrary measurable set in the product σ-algebra,
      -- but h_marginals only gives us equality on cylinder sets (sets determined
      -- by finitely many coordinates).
      --
      -- Standard approach (Kolmogorov extension / π-λ theorem):
      -- 1. Show the product σ-algebra is generated by cylinder sets
      -- 2. Cylinder sets form a π-system (closed under intersection)
      -- 3. Apply h_marginals to show μ_X and μ_Xπ agree on cylinders
      -- 4. Use a uniqueness theorem (e.g., MeasurableSpace.ae_induction_on_inter)
      --    to extend from the generating π-system to all measurable sets
      --
      -- The key missing piece: A mathlib theorem stating that finite measures
      -- agreeing on a generating π-system must agree on the whole σ-algebra.
      -- This requires either:
      -- - Finding the right existing mathlib lemma
      -- - Proving a specialized version for product spaces
      -- - Using Dynkin system machinery to build up the result
      sorry
