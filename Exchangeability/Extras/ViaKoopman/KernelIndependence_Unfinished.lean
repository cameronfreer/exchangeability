...
import Exchangeability.DeFinetti.ViaKoopman.KernelBridge
...
-- Bridge lemmas are imported from KernelBridge.lean:
-- - Kernel.IndepFun.ae_measure_indepFun
-- - Kernel.IndepFun.comp
-- - metProjection_eq_condExpL2_shiftInvariant
-- - condExp_eq_kernel_integral (alias)

/-! ## Regular conditional distribution

NOTE: The definitions `π0`, `measurable_pi0`, `rcdKernel`, `rcdKernel_isMarkovKernel`,
`ν`, and `ν_eval_measurable` are imported from `DirectingKernel.lean`.
-/
...
-- NOTE: `ν_isProbabilityMeasure` and `integral_ν_eq_integral_condExpKernel` are
-- imported from `DirectingKernel.lean`.
