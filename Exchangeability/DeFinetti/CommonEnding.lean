...
lemma rectangles_generate_pi_sigma {m : ℕ} {α : Type*} [MeasurableSpace α] :
    ...
    MeasurableSpace.generateFrom {S : Set (Fin m → α) | ∃ (B : Fin m → Set α),
      (∀ i, MeasurableSet (B i)) ∧ S = {x | ∀ i, x i ∈ B i}} := by
  -- The product σ-algebra is generated by sets of the form {x | x i ∈ B} for each i
  -- Finite intersections of these give rectangles
  -- The σ-algebra generated by rectangles is the same as the σ-algebra generated
  -- by the individual coordinate cylinders
  sorry  -- TODO: Use Pi.measurableSpace definition and generateFrom properties
...
theorem conditional_iid_from_directing_measure
    ...
    ConditionallyIID μ X := by
      ...
      have h_map_prob : IsProbabilityMeasure μ_map := by
        ...
        -- This should be inferrable in mathlib given measurability
        sorry  -- TODO: Prove using Measure.IsProbabilityMeasure instance for map
              -- Likely need: inferInstance with measurability of (fun ω i => X (k i) ω)
      ...
      have h_bind_prob : IsProbabilityMeasure μ_bind := by
        ...
        have h_pi_prob : ∀ ω, IsProbabilityMeasure (Measure.pi fun _ : Fin m => ν ω) := by
          ...
          -- This should follow from Measure.pi instance in mathlib
          sorry  -- TODO: Use inferInstance or Measure.pi.instIsProbabilityMeasure
        sorry  -- TODO: Apply bind probability measure instance with h_pi_prob
