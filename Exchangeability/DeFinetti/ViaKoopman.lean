...
lemma Kernel.IndepFun.comp
  ...
  have hs' : MeasurableSet[MeasurableSpace.comap X inferInstance] s := by
    ...
    have h_le : MeasurableSpace.comap (f ∘ X) inferInstance ≤ MeasurableSpace.comap X inferInstance := by
      -- comap (f ∘ X) is the σ-algebra generated by preimages under (f ∘ X)
      -- comap X is the σ-algebra generated by preimages under X
      -- We need: every set in the first is in the second

      -- The generators of comap (f ∘ X) are {(f ∘ X)⁻¹(U) | U measurable}
      -- But (f ∘ X)⁻¹(U) = X⁻¹(f⁻¹(U))
      -- When f measurable, f⁻¹(U) is measurable, so X⁻¹(f⁻¹(U)) is in comap X

      -- This should follow from comap monotonicity or composition properties
      -- For now, this is a well-defined property that can be proved separately
      sorry
    exact h_le hs
