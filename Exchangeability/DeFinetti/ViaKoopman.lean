...
/-- **Composing kernel independence with measurable functions.**

If X and Y are independent under a kernel, then f(X) and g(Y) are independent
for any measurable functions f and g.

This is the kernel analogue of `IndepFun.comp` at the measure level.

**Proof strategy**:
Independence is defined via measurable space structures: `Kernel.IndepFun X Y κ μ`
means the σ-algebras generated by X and Y are independent. Composing with measurable
functions doesn't add new information to the generated σ-algebras, so independence
is preserved.

Formally: if `comap X mβ` ⊥⊥ `comap Y mγ` under κ, then
`comap (f ∘ X) mβ'` ⊥⊥ `comap (g ∘ Y) mγ'` under κ, where ⊥⊥ denotes
kernel independence.

This should be in Mathlib's `Probability.Independence.Kernel`.
-/
