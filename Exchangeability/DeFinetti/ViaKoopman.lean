...
/-!
### Countable π-system associated to the countable basis on `α`

To upgrade equality on a countable basis to equality of measures we will need a
countable π-system generating the Borel σ-algebra.  This auxiliary section
...
variable (α) [StandardBorelSpace α]

/-- Finite-intersection closure of the countable basis on a standard Borel space. -/
def carrier : Set (Set α) := finiteInterClosure (countableBasis α)

/-- TODO: the carrier is countable (combine the countability of the basis with the
closure under finite intersections). -/
lemma countable : (carrier α).Countable := by
  sorry

/-- TODO: the carrier forms a π-system (use `finiteInterClosure_finiteInter`). -/
lemma isPiSystem : IsPiSystem (carrier α) := by
  sorry

/-- TODO: the σ-algebra generated by the carrier is the ambient one (Borel). -/
lemma generateFrom_eq :
    (inferInstance : MeasurableSpace α) =
      MeasurableSpace.generateFrom (carrier α) := by
  sorry
