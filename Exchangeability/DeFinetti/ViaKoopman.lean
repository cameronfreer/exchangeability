...
lemma Kernel.IndepFun.comp
  ...
  have hs' : MeasurableSet[MeasurableSpace.comap X inferInstance] s := by
    -- hs says s is in the σ-algebra generated by preimages under (f ∘ X)
    -- We need: s is in the σ-algebra generated by preimages under X
    -- This should follow from: every set of the form (f∘X)⁻¹(S) equals X⁻¹(f⁻¹(S))
    -- and when f is measurable, f⁻¹(S) is measurable, so X⁻¹(f⁻¹(S)) is in comap X
    -- TODO: Find the right mathlib lemma for this (likely MeasurableSpace.comap_comp or similar)
    sorry

  have ht' : MeasurableSet[MeasurableSpace.comap Y inferInstance] t := by
    -- Same reasoning as above for t and g
    sorry
