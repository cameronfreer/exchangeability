...
private lemma tail_measurability_of_blockAvg
  ...
  -- Step 2e: Use that α_f is already Measurable (w.r.t. ambient σ-algebra) from L² membership
  -- and the subsequence of Measurable[tailFamily X N] functions converges a.e. to α_f.
  -- By measurable_of_tendsto_metrizable_ae with the countably-generated coarser σ-algebra,
  -- the limit is measurable w.r.t. tailFamily X N.
  --
  -- The key insight: tailFamily X N is generated by X_N, X_{N+1}, ...
  -- The blockAvg functions are sums of f ∘ X_{N+k}, so they're tailFamily-measurable.
  -- The a.e. limit of measurable functions is measurable when the σ-algebra is countably generated
  -- (which it is, since it's generated by countably many random variables).
  --
  -- Final step requires measurable_of_tendsto_metrizable_ae or similar.
  -- TODO: Complete this using the appropriate mathlib lemma for a.e. limits
  sorry
