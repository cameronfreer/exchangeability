...
The `Iic`-indicator base case `directing_measure_integral_Iic_ae_eq_alphaIicCE`
lives in `DirectingMeasureIic` so that the kernel-level bridge file can import
it without dragging in the monotone-class scaffolding here.
