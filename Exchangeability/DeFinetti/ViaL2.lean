...
For a sequence `X : ℕ → Ω → α`, we define:
- `tailFamily X n` := σ(X_n, X_{n+1}, ...) (future σ-algebra from index n onward)
- `tailSigma X` := ⋂_n tailFamily X n (tail σ-algebra)

These σ-algebras form an antitone (decreasing) family and are used to construct
conditional expectations that identify with the L¹ limit from `weighted_sums_converge_L1`.
-/
...
`tailFamily X n = σ(X_n, X_{n+1}, ...)` is the σ-algebra generated by the tail
of the sequence starting at index `n`. This forms an antitone (decreasing) family
as `n` increases. -/
...
`tailSigma X = ⋂_n σ(X_n, X_{n+1}, ...)` is the intersection of all future σ-algebras.
This captures the "asymptotic information" of the sequence. -/
def tailSigma (X : ℕ → Ω → β) : MeasurableSpace Ω :=
  ⨅ n : ℕ, tailFamily X n

/-- The future σ-algebra is antitone: as n grows, we forget earlier variables,
so the σ-algebra gets smaller. -/
lemma antitone_tailFamily (X : ℕ → Ω → β) : Antitone (tailFamily X) := by
  intro n m hnm
  refine iSup_le ?_
  intro k
  -- reindex `m + k` as `n + (k + (m - n))`
  have : m + k = n + (k + (m - n)) := by
    have : m = n + (m - n) := (Nat.add_sub_of_le hnm).symm
    omega
  -- include the `m+k`-th generator into the supremum over `n+•`
  refine le_iSup_of_le (k + (m - n)) ?_
  simp only [this]
  rfl

/-- The tail σ-algebra is contained in each future σ-algebra. -/
lemma tailSigma_le_tailFamily (X : ℕ → Ω → β) (n : ℕ) :
    tailSigma X ≤ tailFamily X n :=
  iInf_le (tailFamily X) n

/-- The tail σ-algebra is a sub-σ-algebra of the ambient space. -/
lemma tailSigma_le (X : ℕ → Ω → β) (hX_meas : ∀ i, Measurable (X i)) :
  ...
  intro s hs
  obtain ⟨t, ht, rfl⟩ := hs
  have : (fun ω => X (0 + k) ω) = X k := by ext ω; simp
  rw [this]
  exact hX_meas k ht
