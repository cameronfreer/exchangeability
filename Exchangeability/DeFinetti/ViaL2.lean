...
lemma directing_measure_measurable
  ...
  ·
    ...
    -- Apply π-λ theorem with π-system = {Iic t | t ∈ ℝ}
    have h_induction : ∀ t (htm : MeasurableSet t), C t htm := by
      apply induction_on_inter
      · -- Borel σ-algebra is generated by Iic sets
        -- This is BorelSpace.measurable_eq but we need the exact form
        -- For now, assert this standard fact
        sorry
      · -- {Iic t | t ∈ ℝ} is a π-system
        -- Need to show: for any Iic s, Iic t, if Iic s ∩ Iic t ≠ ∅, then Iic s ∩ Iic t ∈ the set
        -- Iic s ∩ Iic t = Iic (min s t)
        sorry
      · -- C ∅: ∅ ∈ G
        exact h_empty
      · -- C (Iic t) for all t: Iic t ∈ G for all t
        intro t _
        exact h_pi t
      · -- C t → C tᶜ: G closed under complement
        intro t _ ht
        exact h_compl t ht
      · -- Disjoint countable unions preserved
        intro f hdisj hfm hf_in_G
        exact h_iUnion f hdisj hf_in_G
