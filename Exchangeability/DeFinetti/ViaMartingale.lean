...
lemma contractable_triple_pushforward
  ...
  sorry  -- TODO: Apply Measure.ext using π-λ theorem
         -- Structure:
         -- - h_pi: Rectangles is π-system ✓
         -- - h_agree: measures agree on Rectangles ✓
         -- - Need: Show Rectangles generates the product σ-algebra
         --   (product σ-algebra on (Fin r → α) × α × (Fin k → α) is generated by
         --    sets of the form (pi A) × B × (pi C) where A_i, B, C_j are measurable)
         -- - Need: Both measures are probability measures (follows from isProbabilityMeasure μ)
         -- - Then: Use Measure.ext_of_generateFrom_of_cover or similar π-λ lemma from mathlib
         -- This is a standard but non-trivial application requiring careful mathlib API usage
...
lemma block_coord_condIndep
  ...
  have hiSup_fin :
    ...
    have hge : futureFiltration X m ≤ (⨆ k, finFutureSigma X m k) := by
      -- generators: preimages of cylinders by `shiftRV X (m+1)`
      intro s hs
      rcases hs with ⟨T, hT, rfl⟩
      -- `T` belongs to the Pi σ-algebra; it is generated by finite cylinders,
      -- so there exists `k` such that `T` depends on the first `k` coords.
      -- Standard fact: Pi σ-algebra is generated by cylinder sets depending on finitely many coords.
      -- Therefore `(shiftRV X (m+1)) ⁻¹' T` is in the iSup.
      -- To show: ∃ k, such that this preimage is measurable w.r.t. finFutureSigma X m k
      sorry  -- TODO: Standard - witness k such that T depends only on first k coordinates
             -- Then ⟨T, hT, rfl⟩ gives membership in finFutureSigma X m k
             -- Use le_iSup to lift to the supremum
