  ...
  have h_gen : (inferInstance : MeasurableSpace (α × (ℕ → α)))
    ...
    · -- (⊆) Product σ-algebra ≤ generateFrom S
      ...
      have h_snd_comap : MeasurableSpace.comap (Prod.snd : α × (ℕ → α) → ℕ → α) inferInstance
          ≤ MeasurableSpace.generateFrom S := by
        -- Strategy: Pi σ-algebra is generated by cylinders
        -- Let T = {cylinder r C | r : ℕ, C : Fin r → Set α, all C i measurable}
        -- Then inferInstance on (ℕ → α) = generateFrom T
        -- We've shown Prod.snd⁻¹(t) ∈ generateFrom S for all t ∈ T (via h_snd)
        -- By properties of comap and generateFrom, this implies the result

        -- The key property: comap (generateFrom T) ≤ generateFrom (preimage T)
        -- Since preimage of each cylinder is in generateFrom S, we get comap ≤ generateFrom S
        sorry  -- TODO: Apply comap_generateFrom or similar lemma from mathlib
