              ...
              calc ∫ ω in W ⁻¹' T, φ ω * ψ ω ∂μ
                ...
                _ = ∫ ω in W ⁻¹' T, φ ω * μ[ψ | ℋ] ω ∂μ := by
                    ...
                    have hφ_asm : AEStronglyMeasurable[ℋ] φ μ := by
                      ...
                      -- ℋ is generated by (W, Y), so Y is measurable w.r.t. ℋ
                      have hY_H : @Measurable Ω α ℋ _ Y := by
                        -- ℋ = comap (W, Y) inferInstance
                        -- Y = snd ∘ (W, Y) is measurable w.r.t. comap (W,Y)
                        show Measurable[MeasurableSpace.comap (fun ω => (W ω, Y ω)) inferInstance] Y
                        -- Y ω = ((W ω, Y ω)).snd, so Y = Prod.snd ∘ (fun ω => (W ω, Y ω))
                        have h_eq : Y = Prod.snd ∘ (fun ω : Ω => (W ω, Y ω)) := by
                          ext ω
                          simp [Function.comp_apply]
                        rw [h_eq]
                        -- (W, Y) is measurable in the comap, and Prod.snd is measurable
                        exact measurable_snd.comp (comap_measurable _)
                      -- Now Y ⁻¹' A is ℋ-measurable
                      have hYA_meas : @MeasurableSet Ω ℋ (Y ⁻¹' A) := hY_H hA
                      -- Constant function is ℋ-measurable
                      have hconst : @Measurable Ω ℝ ℋ _ (fun _ : Ω => (1:ℝ)) := @measurable_const ℝ Ω _ ℋ (1:ℝ)
                      -- The indicator is ℋ-measurable
                      exact hconst.indicator hYA_meas
