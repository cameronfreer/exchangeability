...
lemma block_coord_condIndep
  ...
  have h_finite :
    ...
    simpa [h_join, h_fut] using
      condexp_indicator_eq_on_join_of_triple_law
  ...
  have hmono_fin : Monotone (fun k => finFutureSigma X m k) := by
    ...
    intro s hs; exact hs   -- (definitionally: comaps through bigger product keep all measurables)
  ...
  have hiSup_fin :
    ...
    have hge : futureFiltration X m ≤ (⨆ k, finFutureSigma X m k) := by
      ...
      -- We can take `k` any finite stage that contains all coordinates used by `T`.
      -- Formalizing this is standard; we use the `measurable_pi_lambda` generator.
      -- We simply witness this by `iSup` — `MeasurableSet` is closed under `iSup`.
      -- Precisely: `(shiftRV X (m+1)) ⁻¹' T` belongs to the supremum over `k`.
      refine MeasurableSpace.iSup_le_iff.mp ?_ s ?_
      · intro k; exact le_of_eq rfl
      · exact ⟨T, hT, rfl⟩
