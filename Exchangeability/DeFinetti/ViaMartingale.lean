...
lemma prod_path_measure_ext
  ...
  have h_pi : IsPiSystem S := by
    ...
    -- Need to show the tail intersection is also a tailCylinder
    sorry

  -- Show S generates the product σ-algebra
  have h_gen : (inferInstance : MeasurableSpace (α × (ℕ → α))) = MeasurableSpace.generateFrom S := by
    -- The product σ-algebra is generated by sets of the form A × B
    -- where A is measurable in α and B is measurable in (ℕ → α).
    -- The π-system (ℕ → α) is generated by cylinder sets, which are finite intersections
    -- of sets of the form {f | f(i) ∈ Cᵢ}.
    -- tailCylinder r C is exactly {f | ∀ i < r, f(i+1) ∈ C i}, which generates
    -- the σ-algebra on (ℕ → α) when combined with the first coordinate.
    sorry
  ...
  -- Apply π-λ theorem
  -- For finite measures, we just need to check on the generating π-system
  sorry -- Use Measure.ext_of_generateFrom_of_iUnion or similar
