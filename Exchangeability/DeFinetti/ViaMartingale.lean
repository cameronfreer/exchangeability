...
lemma block_coord_condIndep
  ...
  have h_finite :
    ...
    -- Apply the helper lemmas to connect triple law → pair law → CE equality
    sorry  -- TODO: Use join_eq_comap_pair_finFuture and condexp_indicator_eq_on_join_of_triple_law
           -- Structure: h_join := join_eq_comap_pair_finFuture X r m k
           --           h_bridge := condexp_indicator_eq_on_join_of_triple_law Y Zr θk θk' ... h_pair ...
           --           then convert h_bridge using h_join equalities
  ...
  have hmono_fin : Monotone (fun k => finFutureSigma X m k) := by
    intro k ℓ hkℓ; -- adding more coordinates refines the σ-algebra
    sorry  -- TODO: Show comap through larger Fin product refines σ-algebra
  ...
  have hiSup_fin :
    ...
    have hge : futureFiltration X m ≤ (⨆ k, finFutureSigma X m k) := by
      ...
      sorry  -- TODO: Standard - T depends on finitely many coords, witness by iSup
  ...
  have h_up_left :
    ...
    sorry  -- TODO: Apply condExp_tendsto_iSup (Lévy upward)
  have h_up_right :
      Tendsto (fun k => μ[Set.indicator B (fun _ => (1 : ℝ)) ∘ Y
  ...
  -- their limits are a.e.-equal; use unique limit + levelwise equality
  sorry  -- TODO: Apply tendsto_nhds_unique with h_up_left, h_up_right, and h_finite
         -- to conclude the two limits are a.e. equal
