    ...
    have h_gen : firstRSigma X r ⊔ finFutureSigma X m k = MeasurableSpace.generateFrom CylinderSets := by
      ...
      · -- (⊆) Show sup ≤ generateFrom CylinderSets
        ...
        · -- firstRSigma X r ≤ generateFrom CylinderSets
          intro E hE
          obtain ⟨A, hA, rfl⟩ := hE
          sorry -- TODO (~15-20 min): Show firstRSigma preimages are in generateFrom Cylinders
                -- Type issue: A : Set (Fin r → α) but need Fin r → Set α
                -- Strategy: Show each coordinate X i is measurable by constructing isolating cylinders
        · -- finFutureSigma X m k ≤ generateFrom CylinderSets
          intro E hE
          obtain ⟨C, hC, rfl⟩ := hE
          sorry -- TODO (~15-20 min): Symmetric to firstRSigma case
