...
lemma measure_ext_of_future_rectangles
  ...
  have h_gen : (inferInstance : MeasurableSpace (α × (ℕ → α)))
    ...
    -- Standard result in measure theory: The product σ-algebra on α × (ℕ → α)
    -- is generated by measurable rectangles B ×ˢ E where:
    -- - B is a measurable subset of α
    -- - E is a measurable subset of (ℕ → α)
    --
    -- Since the Pi σ-algebra on (ℕ → α) is generated by cylinder sets
    -- (sets depending on finitely many coordinates), and our S consists of
    -- all rectangles B ×ˢ (cylinder r C), we have that S generates the
    -- product σ-algebra.
    --
    -- Proof sketch:
    -- (⊇) Every set in S is a measurable rectangle, so generateFrom S ≤ product σ-algebra
    -- (⊆) Every measurable rectangle B ×ˢ E can be approximated by sets in S
    --     since E (being measurable in Pi σ-algebra) is generated by cylinders
    sorry
