...
lemma contractable_triple_pushforward
  ...
  have h_gen : (inferInstance : MeasurableSpace ((Fin r → α) × α × (Fin k → α)))
    ...
    sorry  -- TODO: Prove product σ-algebra = generateFrom Rectangles
           -- The product σ-algebra on (Fin r → α) × α × (Fin k → α) equals
           -- the σ-algebra generated by sets of form (pi A) × B × (pi C)
           -- This is similar to the proof at lines 969-1124 but for this specific product
