    ...
    have h_gen : firstRSigma X r ⊔ finFutureSigma X m k = MeasurableSpace.generateFrom CylinderSets := by
      ...
      · -- (⊆) Show sup ≤ generateFrom CylinderSets
        ...
        · -- firstRSigma X r ≤ generateFrom CylinderSets
          intro E hE
          obtain ⟨A, hA, rfl⟩ := hE
          sorry -- TODO (~20-30 min): Show comap-measurable sets ∈ generateFrom Cylinders
                -- **Mathematical fact:** Product σ-algebra on Fin n → α is generated by cylinder sets
                -- **Have:** A : Set (Fin r → α), MeasurableSet A
                --         E = (firstRMap X r)⁻¹(A)
                -- **Need:** E ∈ generateFrom CylinderSets
                --
                -- **Strategy:**
                -- 1. Use that product σ-algebra = generateFrom {rectangles}
                --    Mathlib: MeasurableSpace.pi = generateFrom (pi_generateFrom_eq)
                -- 2. Show A can be written as countable union/intersection of cylinders
                --    (Dynkin/monotone class argument on product space)
                -- 3. Pull back through firstRMap to get E in generated σ-algebra
                --
                -- **Blocker:** Requires deep dive into mathlib product σ-algebra lemmas
                --   - Need: generateFrom_pi or similar
                --   - May need: auxiliary Dynkin argument on Fin r → α
        · -- finFutureSigma X m k ≤ generateFrom CylinderSets
          intro E hE
          obtain ⟨C, hC, rfl⟩ := hE
          sorry -- TODO (~20-30 min): Symmetric to firstRSigma case
                -- Same mathematical strategy as above, applied to future coordinates
                --
                -- **Have:** C : Set (Fin k → α), MeasurableSet C
                --         E = (futureMap X m k)⁻¹(C)
                -- **Need:** E ∈ generateFrom CylinderSets
                --
                -- Once firstRSigma case is solved, this follows the same pattern
