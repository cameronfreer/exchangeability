  ...
  have h_gen : (inferInstance : MeasurableSpace (α × (ℕ → α)))
    ...
    · -- (⊆) Product σ-algebra ≤ generateFrom S
      ...
      have h_snd_comap : MeasurableSpace.comap (Prod.snd : α × (ℕ → α) → ℕ → α) inferInstance
        ...
        -- Use measurable_generateFrom: function is measurable if preimages of
        -- generating sets are measurable
        --
        -- Pi σ-algebra is generated by cylinders, and we've shown (h_snd) that
        -- Prod.snd⁻¹(cylinder r C) is measurable in generateFrom S for all cylinders
        --
        -- However, we need to formally state that Pi is generated by cylinders
        -- and apply measurable_generateFrom with that generating set
