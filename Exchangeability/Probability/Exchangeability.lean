...
theorem exchangeable_iff_fullyExchangeable {μ : Measure Ω} {X : ℕ → Ω → α}
  ...
  · intro hexch
    ...
    have h_unique : μ_X = μ_Xπ := by
      ...
      ext S hS
      -- For measurable S in the product σ-algebra, we need μ_X S = μ_Xπ S
      -- The product σ-algebra is generated by cylinder sets (finite coordinate sets)
      -- Use a generating π-system argument:
      sorry
      -- Detailed steps needed:
      -- a) Identify the π-system of cylinders generating MeasurableSpace.pi
      -- b) Show μ_X and μ_Xπ agree on this π-system (via h_marginals)
      -- c) Apply Dynkin's π-λ theorem / uniqueness of extensions
      -- d) Relevant mathlib: MeasureTheory.pi_eq_generateFrom, Measure.ext_of_generate_finite
