...
lemma condExp_indicator_eq_of_law_eq_of_comap_le
  ...
  **Proof outline (Kallenberg L² argument):**
  ...
  -- IMPORTANT: Compute ρ = law(W) = law(W') and define ρ as alias BEFORE
  -- introducing let-definitions for MeasurableSpace to avoid instance shadowing.
  -- Use explicit @ notation to lock in the measurable space instances.
  ...
  have h_sq_eq : ∫ ω, μ₁ ω * μ₁ ω ∂μ = ∫ ω, μ₂ ω * μ₂ ω ∂μ := by
    /-
    TODO: Complete using Doob-Dynkin factorization + set-integral uniqueness.

    **Proof outline (mathematically complete):**
    1. Doob-Dynkin factorization (from Mathlib.MeasureTheory.Function.FactorsThrough):
       - μ₁ = g₁ ∘ W for some measurable g₁ : γ → ℝ (via hμ₁_sm.exists_eq_measurable_comp)
       - μ₂ = g₂ ∘ W' for some measurable g₂ : γ → ℝ

    2. Show g₁ = g₂ ρ-a.e. (via Integrable.ae_eq_of_forall_setIntegral_eq):
       Both g₁ and g₂ satisfy the same set-integral equation:
       ...
       So g₁ = g₂ ρ-a.e. by uniqueness of functions determined by set integrals.

    3. Push squares through integral_map:
       ∫ μ₁² dμ = ∫ (g₁ ∘ W)² dμ = ∫ g₁² dρ = ∫ g₂² dρ = ∫ (g₂ ∘ W')² dμ = ∫ μ₂² dμ

    **Technical notes:**
    - ρ = Measure.map W μ = Measure.map W' μ (from hρ_eq/marginal_law_eq_of_pair_law)
    - The API issues involve MeasurableSpace instance shadowing from let definitions
    - StronglyMeasurable.pow_const requires explicit function calls rather than dot notation
    -/
    -- For now, using sorry while API issues are resolved
    sorry
