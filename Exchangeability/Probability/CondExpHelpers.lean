...
theorem condExp_project_of_condIndepFun
  ...
  have h_all : ∀ (T : Set Ω), MeasurableSet[mZW] T → μ T < ∞ →
    ...
    have h𝓡_pi : IsPiSystem 𝓡 := by
      sorry
      /-
      Need to show: ∀ S T ∈ 𝓡, S ∩ T ≠ ∅ → S ∩ T ∈ 𝓡
      If S = Z⁻¹(A₁) ∩ W⁻¹(B₁) and T = Z⁻¹(A₂) ∩ W⁻¹(B₂), then:
      S ∩ T = (Z⁻¹(A₁) ∩ W⁻¹(B₁)) ∩ (Z⁻¹(A₂) ∩ W⁻¹(B₂))
            = Z⁻¹(A₁ ∩ A₂) ∩ W⁻¹(B₁ ∩ B₂)
      which is in 𝓡 since A₁ ∩ A₂ and B₁ ∩ B₂ are measurable.
      -/
    ...
    have h𝓡_gen : MeasurableSpace.generateFrom 𝓡 = mZW := by
      sorry
      /-
      This follows from the general fact that mZ ⊔ mW is generated by
      rectangles from the two σ-algebras.

      Key: mZ ⊔ mW is the smallest σ-algebra containing both mZ and mW.
      The rectangles Z⁻¹(A) ∩ W⁻¹(B) include:
      - Sets Z⁻¹(A) ∩ W⁻¹(univ) = Z⁻¹(A) (gives mZ)
      - Sets Z⁻¹(univ) ∩ W⁻¹(B) = W⁻¹(B) (gives mW)
      So generateFrom 𝓡 contains both mZ and mW, hence contains mZ ⊔ mW.

      Conversely, each rectangle is mZ ⊔ mW-measurable by closure under ∩.
      Therefore generateFrom 𝓡 ⊆ mZ ⊔ mW.
      -/
    ...
    have h_rect_all : ∀ (R : Set Ω), R ∈ 𝓡 → μ R < ∞ →
      ...
      exact h_rect (W ⁻¹' B) hmW_preimage hμR A hA
    ...
    case compl =>
      ...
      sorry
      /-
      Have: hS'_C : μ S' < ∞ → ∫_S' g = ∫_S' f(Y)
      Want: ∫_Sᶜ g = ∫_Sᶜ f(Y)

      By IsProbabilityMeasure: μ Ω = 1 < ∞, so we can apply hS'_C
      ∫_Sᶜ g = ∫_Ω g - ∫_S' g          (integral_diff)
             = ∫_Ω f(Y) - ∫_S' f(Y)    (by hS'_C applied to Ω and S')
             = ∫_Sᶜ f(Y)
      -/
    ...
    case iUnion =>
      ...
      sorry
      /-
      Have: ∀ n, hSeq_C n : μ (Sseq n) < ∞ → ∫_(Sseq n) g = ∫_(Sseq n) f(Y)
      Want: ∫_(⋃ n, Sseq n) g = ∫_(⋃ n, Sseq n) f(Y)

      By finite measure: μ (⋃ Sₙ) < ∞ implies ∑ μ (Sₙ) < ∞
      So μ (Sₙ) < ∞ for each n, can apply hSeq_C n

      ∫_(⋃ Sₙ) g = ∑ ∫_(Sₙ) g           (integral_iUnion for disjoint)
                 = ∑ ∫_(Sₙ) f(Y)         (by hSeq_C n for each n)
                 = ∫_(⋃ Sₙ) f(Y)
      -/
    /-
